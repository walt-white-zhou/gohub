// Package verifycode 用以发送手机验证和邮箱验证码
package verifycode
...
func (vc *VerifyCode) SendSMS(phone string) bool {
	...
	return sms.NewSMS().Send(phone, sms.Message{
		Template: config.GetString("sms.aliyum.template_code"),
		Data:     map[string]string{"code": code},
	})
}

// CheckAnswer 检查用户提交的验证码是否正确，key可以是手机或者 Email
...
func (vc *VerifyCode) generateVerifyCode(key string) string {
	...
	// 生成随机码
	code := helpers.RandNumber(config.GetInt("verifycode.code_length"))
	...
	// 将验证码及key(邮箱或手机号)存放到Redis中并设置过期时间
