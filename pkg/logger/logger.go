...
// InitLogger 日志初始化
func InitLogger(filename string, maxSize, maxBackup, maxAgs int, compress bool, logType string, level string) {

	// 获取日志写入介质
	writeSyncer := getLogWriter(filename, maxSize, maxBackup, maxAgs, compress, logType)

	// 设置日志等级，具体请见 config/log.go文件
	logLevel := new(zapcore.Level)
	if err := logLevel.UnmarshalText([]byte(level)); err != nil {
		fmt.Println("日志初始化错误，日志级别设置有误。请修改 config/log.go 文件中的log.level 配置项")
	}
...
func getEncoder() zapcore.Encoder {
	...
	encoderConfig := zapcore.EncoderConfig{
		...
		LineEnding:     zapcore.DefaultLineEnding,      // 每行日志的结尾加 "\n"
...
func getLogWriter(filename string, maxSize, maxBackup, maxAgs int, compress bool, logType string) zapcore.WriteSyncer {
