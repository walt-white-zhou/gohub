...
func (jwt *JWT) ParserToken(c *gin.Context) (*JWTCustomClaims, error) {
	...
	// 3. 将 token 中的 claims 信息解析出来和JWTCustomClaims 数据结构进行校验
...
// RefreshToken 更新 Token, 用于提供 refresh token 接口
...
// createToken 创建token，内部使用，外部调用 IssueToken
...
// getTokenFromHeader 使用 jwtpkg.ParseWithClaims 解析token
